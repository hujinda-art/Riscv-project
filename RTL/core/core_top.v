...
//--------------------------------------------------------------------------------
`include "../module/branch_predictor.v"
//--------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
    ...
    assign branch_resolved = ex_is_branch && ex_instr_valid_out;
    assign ex_branch_mispredict = branch_resolved && (ex_branch_taken != ex_predict_taken);
    assign btb_update = branch_resolved && ex_branch_taken;
    assign btb_update_pc = ex_pc_out;
    assign btb_update_target = ex_pc_branch;
...
//--------------------------------------------------------------------------------
    ...
    assign predict_branch = instr_valid_out && id_is_branch && predict_taken;
...
//-------------------------------------------------------------------------------
    ...
    IF_stage u_if (
        ...
        .pc_jump(pc_jump_if),
//----------------------------------------------------------------------------
//删除.branch(ex_branch_taken) .pc_branch(ex_pc_banch)
        .branch(ex_is_branch && ex_instr_valid_out),
        .pc_branch(ex_branch_taken ? ex_pc_branch : ex_pc_out + 32'd4),
...
//----------------------------------------------------------------------------
        .predict_taken_in(predict_branch),
//----------------------------------------------------------------------------
        .ex_pc(ex_pc_out),
